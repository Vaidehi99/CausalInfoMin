...
class LXRTModel(BertPreTrainedModel):
    ...
    def __init__(self, config, reweigh_lang=False, reweigh_vision=False):
        super().__init__(config)
        self.embeddings = BertEmbeddings(config)
        self.encoder = LXRTEncoder(config)
        self.pooler = BertPooler(config)

        self.reweigh_lang = reweigh_lang
        if self.reweigh_lang:
            self.lang_gate_fc = nn.Linear(768, 768)
        self.reweigh_vision = reweigh_vision
        if self.reweigh_vision:
            self.vision_fate_fc = nn.Linear(768, 768)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None,
        ...
        if self.reweigh_lang:
            lang_gate = nn.functional.sigmoid(self.lang_gate_fc(embedding_output))
            embedding_output = lang_gate * embedding_output
        if self.reweigh_vision:
            vision_gate = nn.functional.sigmoid(self.vision_fate_fc(visual_feats[0]))
            visual_feats_0 = vision_gate * visual_feats_0
...
class LXRTFeatureExtraction(BertPreTrainedModel):
    ...
    """
    def __init__(self, config, mode='lxr', reweigh_lang=False, reweigh_vision=False):
        """
        ...
        """
        super().__init__(config)
        self.bert = LXRTModel(config, reweigh_lang=reweigh_lang, reweigh_vision=reweigh_vision)
        self.mode = mode
        self.apply(self.init_bert_weights)
