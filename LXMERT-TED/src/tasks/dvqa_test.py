...
def get_logits(model, dataloader):
    ...
    for v, b, q, a, i in tqdm(iter(dataloader)):

        batch_size = v.size(0)
        v = v.cuda()
        b = b.cuda()
        logits = model(v, b, list(q))
        pred[idx:idx + batch_size, :].copy_(logits['logit'].data)
...
if __name__ == '__main__':
    ...
    if opt.causal_model:
        print("Bias dim factor: %s", opt.bias_dim_factor)
        model = CausalVQAModel(2274, bias_dim_factor=opt.bias_dim_factor,
                                    pooled_layer_norm=opt.use_farm, contrastive=opt.contrastive,
                                    tie_training=opt.tie_training)
    else:
        model = VQAModel(2274)
    model = model.cuda()
