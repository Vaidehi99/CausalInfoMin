...
def _load_dataset(dataroot, name, label2ans,ratio=1.0):
    ...
    # question_path = os.path.join(dataroot, 'vqacp_v2_probe_punct_%s_questions.json' % (name))

    # question_path = os.path.join(dataroot, 'vqacp_v2_vqa_original_%s_questions.json' % (name))
    # m=json.load(open(question_path))
    # print(m['questions'][0])
    # exit()
...
class VQAFeatureDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        np_feature = np.frombuffer(feature, dtype=np.float32).reshape(2048, 36)
        # features = torch.from_numpy(np.frombuffer(feature, dtype=np.float32).reshape(2048, 36)).permute(1, 0)
        features = torch.from_numpy(np.copy(np_feature)).permute(1, 0)
        ...
        boxes = torch.from_numpy(np.copy(np.frombuffer(box, dtype=np.float32).reshape(4, 36))).permute(1, 0)
